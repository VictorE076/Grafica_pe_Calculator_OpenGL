...
/// 2. Observatorul este initial in (0, 0, -800) si se uita in sus spre (0, 0, 800)
...
/// 2. SpecialKeys: 'LEFT', 'RIGHT', 'UP', 'DOWN';
...
void CreateVBO(void)
{
	...
	GLfloat Vertices[] =
	{
		...
		*/
		...
		/// 2. Desenare TETRAEDRU
		/// Varfurile din planul z = -50.f (Cele 3 varfuri care alcatuiesc fata triunghiulara "de jos");
		/// Coordonate;					
		-50.f, -50.f, -50.f, 1.f,	/// indice 0
		50.f, -50.f, -50.f, 1.f,	/// indice 1 
		0.f, 50.f, -50.f, 1.f,		/// indice 2

		/// Varful din planul z = 50.f (Varful "de sus");
		/// Coordonate;					
		0.f, 0.f, 50.f, 1.f,		/// indice 3
	};

	//	CULORILE instantelor;
	///	2. Culorile sunt generate in functie de indexul de instantiere - fiecare cub sau tetraedru va avea o singura culoare;
	...
	/// 2. MATRICELE instantelor-se defineste tiparul de desenare al cuburilor sau al tetraedrelor prin ROTIREA si TRANSLATIA cubului initial (INSTANCED RENDERING);
	glm::mat4 MatModel[INSTANCE_COUNT];
	for (int instID = 0; instID < INSTANCE_COUNT; instID++)
	{
		/// 2. Curba 3D => c(t) = (90 * t * sin(t), 90 * t * cos(t), 90 * t * cos(t)):
		MatModel[instID] =
			glm::translate(glm::mat4(1.0f), glm::vec3(90 * instID * sin(instID),								///	2. Deplasarea pe OX;
													  90 * instID * cos(instID),								///	2. Deplasarea pe OY; 
													  /*90 * instID*/ 90 * instID * cos(instID) /*0.f*/))		/// 2. Deplasarea pe OZ;
...
void RenderFunction(void)
{
	...
	*/
	...
	/// 2. Desenarea fetelor TETRAEDRULUI;
	codCol = 0; // Culorile INSTANTELOR
	glUniform1i(codColLocation, codCol);
	glDrawElementsInstanced(GL_TRIANGLES, 12, GL_UNSIGNED_INT, (void*)(0), INSTANCE_COUNT);

	
	//  Desenarea muchiilor CUBULUI;
	/*
	...
	*/
	...
	/// 2. Desenarea muchiilor TETRAEDRULUI;
	codCol = 1; // Black
	glUniform1i(codColLocation, codCol);
	glDrawElementsInstanced(GL_LINE_LOOP, 3, GL_UNSIGNED_INT, (void*)(0), INSTANCE_COUNT);
	glDrawElementsInstanced(GL_LINES, 6, GL_UNSIGNED_INT, (void*)(12 * sizeof(GLuint)), INSTANCE_COUNT);
