...
void CreateVBO(void)
{
	...
	GLfloat Vertices[] = {
		//	Patrat "mare" (sens orar => GL_BACK)
		 0.5f, -0.5f, 0.0f, 1.0f,
		-0.5f, -0.5f, 0.0f, 1.0f,
		-0.5f, 0.5f, 0.0f, 1.0f,
		 0.5f, 0.5f, 0.0f, 1.0f,
	   //	Patrat "mic" (sens trigonometric => GL_FRONT)
		 0.3f, 0.3f, 0.0f, 1.0f,
		-0.3f, 0.3f, 0.0f, 1.0f,
		-0.3f, -0.3f, 0.0f, 1.0f,
		 0.3f, -0.3f, 0.0f, 1.0f
...
void RenderFunction(void)
{
	...
	glPolygonMode(GL_FRONT, GL_FILL);		//	Poligon cu fata => umplere (patratul mic);
	glPolygonMode(GL_BACK, GL_LINE);		//	Poligon cu spatele => contur (patratul mare);
	...
	glDrawArrays(GL_TRIANGLE_FAN, 0, 4); // Patratul mare
	glDrawArrays(GL_QUADS, 4, 4); // Patratul mic
...
void UseMouse(int button, int state, int x, int y)
{
	...
	case GLUT_RIGHT_BUTTON:			//	CLICK dreapta => variabila codCol este 0;
		if (state == GLUT_DOWN)
		{
			codCol = 0; // change to GLfloat colors[] = {...} in this cpp file
			glUniform1i(codColLocation, codCol);
		}
		break;
	default:
