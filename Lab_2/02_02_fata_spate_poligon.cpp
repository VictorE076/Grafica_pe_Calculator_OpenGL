...
int codCol;	// '0' Default (Variabila globala) 	
...
void CreateVBO(void)
{
	///// !!! Coordonatele triunghiului "mic" trebuie sa fie dupa (peste) corrdonatele triunghiului "mare" pentru a evita "overlapping"
...
void Initialize(void)
{
	...
	glPolygonMode(GL_FRONT, GL_FILL); // Modul de desenare in functie de orientare - ex: cu fata => deseneaza triunghiul complet (umplere);
	glPolygonMode(GL_BACK, GL_LINE); // Modul de desenare in functie de orientare - ex: cu spatele => deseneaza conturul triunghiului;
...
void RenderFunction(void) ///// !!! RenderFunction is always "looping" inside Main 
{
...
void UseMouse(int button, int state, int x, int y)
{
	...
	case GLUT_LEFT_BUTTON:			//	CLICK stanga => variabila codCol este 0;
		if (state == GLUT_DOWN)
		{
			glFrontFace(GL_CW); // selects clockwise polygons as front-facing (sens ceasornic => fata, sens trigonometric => spate);

			codCol = 0; 
		}
		break;

	case GLUT_RIGHT_BUTTON:			//	CLICK dreapta => variabila codCol este 1;
		if (state == GLUT_DOWN)
		{
			glFrontFace(GL_CCW); // selects counter-clockwise polygons as front-facing (sens ceasornic => spate, sens trigonometric => fata);
