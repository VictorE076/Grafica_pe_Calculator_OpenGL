...
/// 2. Elemente pentru reprezentarea suprafetei
/// (1) intervalele convenabile pentru parametrii considerati (u si v)
/// pentru a desena un CILINDRU:
const float U_MIN = -PI, U_MAX = PI;
const float V_MIN = -30 * PI, V_MAX = 30 * PI;
...
void CreateVBO(void)
{
	...
	for (int merid = 0; merid < NR_MERID; merid++)
	{
		for (int parr = 0; parr < NR_PARR + 1; parr++)
		{
			...
			/// 2. Coordonatele varfului corespunzator lui (u, v)
			...
			Colors[index] = glm::vec3(0.1f + cosf(u), 0.1f + sinf(u), 0.1f + v);
			Indices[index] = index;
