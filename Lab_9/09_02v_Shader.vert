...
void main(void)
{
    ...
    vec3 Normal = vec3(projection * view * vec4(in_Normal, 0.f));
    vec3 inLightPos = vec3(projection * view * vec4(lightPos, 1.f));
    ...
    /// 2. Am modificat "ambientStrength" de la "0.2f" la "1.5f", astfel incat lumina sa bata mult mai puternic in optiunile: 
    /// "Varfuri" si "Varfuri+Mediere_Normale".
